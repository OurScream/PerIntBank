package perinfo;

public class person {
	private String zjid; //证件号（限制为3位）
	private String khdlmm;//客户登录密码
	private String khxm;//客户姓名
	private String csrq;//出生日期
	private String hyzk;//婚姻状况
	private String ssmz;//所属民族
	private String zjlx;//证件类型
	private String zjqsrq;//证件有效起始日期
	private String zjjzrq;//证件有效截止日期
	private String khxb;//客户性别
	private String khgj;//客户国籍
	private String yddh;//移动电话
	private String gddh;//固定电话
	private String jtdz;//家庭地址
	private String jtyb;//家庭邮编
	public String getZjid() {
		return zjid;
	}
	public void setZjid(String zjid) {
		this.zjid = zjid;
	}
	public String getKhdlmm() {
		return khdlmm;
	}
	public void setKhdlmm(String khdlmm) {
		this.khdlmm = khdlmm;
	}
	public String getKhxm() {
		return khxm;
	}
	public void setKhxm(String khxm) {
		this.khxm = khxm;
	}
	public String getCsrq() {
		return csrq;
	}
	public void setCsrq(String csrq) {
		this.csrq = csrq;
	}
	public String getHyzk() {
		return hyzk;
	}
	public void setHyzk(String hyzk) {
		this.hyzk = hyzk;
	}
	public String getSsmz() {
		return ssmz;
	}
	public void setSsmz(String ssmz) {
		this.ssmz = ssmz;
	}
	public String getZjlx() {
		return zjlx;
	}
	public void setZjlx(String zjlx) {
		this.zjlx = zjlx;
	}
	public String getZjqsrq() {
		return zjqsrq;
	}
	public void setZjqsrq(String zjqsrq) {
		this.zjqsrq = zjqsrq;
	}
	public String getZjjzrq() {
		return zjjzrq;
	}
	public void setZjjzrq(String zjjzrq) {
		this.zjjzrq = zjjzrq;
	}
	public String getKhxb() {
		return khxb;
	}
	public void setKhxb(String khxb) {
		this.khxb = khxb;
	}
	public String getKhgj() {
		return khgj;
	}
	public void setKhgj(String khgj) {
		this.khgj = khgj;
	}
	public String getYddh() {
		return yddh;
	}
	public void setYddh(String yddh) {
		this.yddh = yddh;
	}
	public String getGddh() {
		return gddh;
	}
	public void setGddh(String gddh) {
		this.gddh = gddh;
	}
	public String getJtdz() {
		return jtdz;
	}
	public void setJtdz(String jtdz) {
		this.jtdz = jtdz;
	}
	public String getJtyb() {
		return jtyb;
	}
	public void setJtyb(String jtyb) {
		this.jtyb = jtyb;
	}
}
